...
const singinParticipant = async (req) => {
...
const getAllEvents = async () => {
  const result = await Events.find({ statusEvent: "Published" })
    .select("_id title date tickets venueName price")
    .populate("image")
    .populate("category");
  return result;
};
...
const getAllOrders = async (req) => {
  ...
  const check = await Participant.findOne({ _id: id });
  if (!check)
    throw new NotFoundError(`${req.participant.email} bukan participant`);
...
const getPayment = async (req) => {
  const result = await Payments.find({ organizer: req.params.organizer }).populate("image");
  return result;
};
